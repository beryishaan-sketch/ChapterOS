...
const sendEmail = async ({ to, subject, html, text }) => {
  try {
    ...
    const info = await transporter.sendMail({
      from: `"ChapterOS" <${process.env.SMTP_USER}>`,
      to,
      subject,
      html,
      text,
  ...
  const html = `
    <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #0F1C3F; padding: 24px; text-align: center;">
        <h1 style="color: #C9A84C; margin: 0;">ChapterOS</h1>
      ...
      <div style="padding: 32px; background: #fff;">
        ...
        <p>Please log in to ChapterOS to complete your payment.</p>
...
const sendMemberInvite = async ({ to, orgName, inviteToken }) => {
  const html = `
    <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 32px; background: #fff;">
        ...
        <p>Your chapter is using ChapterOS to manage operations. Click below to create your account.</p>
  ...
  `;
  return sendEmail({ to, subject: `You've been invited to join ${orgName} on ChapterOS`, html });
};
